...
try:
    ...
    print(
        ">> [Warning] VulkanMatrixMultiply module not found. GPU acceleration will be disabled."
    )
...
QDRANT_COLLECTION_NAME = "dialogue_pairs_v3_qwen"
# The new model uses 1024 dimensions.
...
# (在 risk_db_lock = threading.Lock() 这一行的下面)
vulkan_engine = None
...
def setup_vector_database():
    ...
    try:
        ...
        # --- ROBUST MODEL PATH ---
        model_path = str(Path(__file__).parent / "qwen3-embedding-0.6b-local")
...
def benchmark_vectorization(text, N=1024, M=1024):
    """
    【性能对比核心函数】
    接收一段文本，然后同时使用CPU和GPU进行向量化，并返回结果。
    - N: 输出向量的维度
    - M: 输入向量的维度 (模拟)
    """
    print(f"\n{Colors.YELLOW}--- [Vectorization Benchmark Started] ---{Colors.ENDC}")

    # --- 1. CPU (SentenceTransformer) ---
    print(
        f"{Colors.CYAN}>> [CPU Benchmark] Vectorizing with SentenceTransformer...{Colors.ENDC}"
    )
    cpu_start_time = time.time()
    cpu_vector = EMBEDDING_MODEL.encode(text).tolist()  # 这是您原来的CPU方法
    cpu_end_time = time.time()
    cpu_duration_ms = (cpu_end_time - cpu_start_time) * 1000
    print(f"   -> {Colors.GREEN}CPU finished in: {cpu_duration_ms:.4f} ms{Colors.ENDC}")

    # --- 2. GPU (VulkanEngine) ---
    gpu_vector = None
    if VULKAN_AVAILABLE and vulkan_engine:
        print(
            f"{Colors.MAGENTA}>> [GPU Benchmark] Vectorizing with VulkanEngine...{Colors.ENDC}"
        )

        # 为了模拟一个真实的、计算量大的任务，我们这样做：
        # a. 将文本转换为一个固定长度的输入向量
        input_vector = [float(ord(c)) for c in text[:M]]
        input_vector.extend([0.0] * (M - len(input_vector)))  # 补齐

        # b. 创建一个巨大的、随机的权重矩阵来模拟Transformer模型
        #    这个矩阵乘法是计算的核心
        #    注意：我们使用numpy来高效地创建这个随机矩阵
        weight_matrix = np.random.rand(N, M).astype(np.float32).flatten().tolist()

        # c. 调用您的Vulkan模块！
        gpu_start_time = time.time()
        # N=输出维度, M=输入维度, P=1 (因为我们输入的是向量)
        gpu_vector = vulkan_engine.multiply(weight_matrix, input_vector, N, M, 1)
        gpu_end_time = time.time()
        gpu_duration_ms = (gpu_end_time - gpu_start_time) * 1000
        print(
            f"   -> {Colors.GREEN}GPU finished in: {gpu_duration_ms:.4f} ms{Colors.ENDC}"
        )
    else:
        print(
            f"{Colors.RED}   -> GPU (VulkanEngine) is not available, skipping.{Colors.ENDC}"
        )

    # --- 3. 结果对比 ---
    if gpu_vector:
        speedup = (
            cpu_duration_ms / gpu_duration_ms if gpu_duration_ms > 0 else float("inf")
        )
        print(
            f"\n{Colors.YELLOW}>> [Benchmark Result] GPU was {speedup:.2f} times faster than CPU.{Colors.ENDC}"
        )

    print(f"{Colors.YELLOW}-------------------------------------------{Colors.ENDC}")

    # 默认情况下，我们仍然返回由sentence-transformer生成的、语义正确的向量
    # 因为我们的Vulkan实现只是为了模拟计算量，而不是为了语义正确性
    return cpu_vector
...
# ... (In Section 5, after another workflow function ends) ...
...
def advisor_task(current_topic):
    ...
    try:
        ...
        # 这是新的、会进行性能对比的代码行
        print(
            f"   -> [BG] Running benchmark for advisor query (length: {len(current_topic)})..."
        )
        query_vector = benchmark_vectorization(current_topic)
        ...
        # We can also reduce the limit to make it faster
        search_results = QDRANT_CLIENT.search(
            collection_name=QDRANT_COLLECTION_NAME,
            query_vector=query_vector,
            limit=3,  # Let's reduce this from 5 to 3 for a faster response
        )
...
def process_metadata_and_vectorize(record_id, stage):
    ...
    try:
        ...
        if output_text and IS_QDRANT_DB_READY:
            ...
            vector = benchmark_vectorization(full_text)
...
if __name__ == "__main__":
    ...
    if VULKAN_AVAILABLE:
        print(
            f"{Colors.BLUE}>> [System] Initializing Vulkan Compute Engine...{Colors.ENDC}"
        )
        try:
            vulkan_engine = VulkanMatrixMultiply.VulkanEngine()
            print(
                f"✅ {Colors.GREEN}[System] Vulkan Compute Engine is ready.{Colors.ENDC}"
            )
        except Exception as e:
            vulkan_engine = None
            VULKAN_AVAILABLE = False  # 如果失败，更新状态
            print(
                f"❌ {Colors.RED}[Vulkan Engine Error] Initialization failed: {e}{Colors.ENDC}"
            )
    # --- 【【【 初始化结束 】】】 ---
