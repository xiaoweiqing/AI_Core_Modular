...
# ==============================================================================
# ### --- START: JOB MATCHING CORE WORKFLOW --- ###
import shutil # 确保在文件顶部导入 shutil
import fitz # PyMuPDF, 如果需要支持PDF的话
import os
...
class Colors:
    ...
    MAGENTA = "\033[95m"
    BLUE = "\033[94m"
    ENDC = "\033[0m" # ENDC 和 RESET 作用一样，保留是为了兼容性
...
# ### --- START: JOB MATCHING LOGIC CONFIGURATION --- ###

# 1. 文件夹路径 (这是核心)
# IPC_DIR 已经存在，我们复用它
INBOX_DIR = IPC_DIR / "inbox"              # 【核心】简历监控入口文件夹
ARCHIVE_DIR = IPC_DIR / "archive"            # 【核心】已处理简历的归档文件夹
# 1. 文件夹路径 (这是核心)
# ...
PROJECT_DIR = Path(__file__).resolve().parent # 获取当前脚本所在的目录
JDS_FOLDER_PATH = PROJECT_DIR / "JDs_library" # 【核心】JD存放文件夹
# ...

# 2. 专属数据库配置
JOB_MATCHING_DB = Path.home() / "job_matching_data.sqlite"
JOB_MATCHING_TABLE_NAME = "job_reports"
job_matching_db_lock = threading.Lock()

# 3. 专属Qdrant集合配置
QDRANT_JOB_REPORTS_COLLECTION = "ai_job_reports_v1"

# 4. 【核心】与您原程序完全一致的AI指令 (Prompt)
PROMPT_ANALYSIS_REPORTER = """
# 角色
你是一位极其苛刻、一针见血的顶级人才战略分析师。你的核心任务是，将候选人简历与职位要求(JD)进行严格的逐点审查。

# 核心分析流程 (必须严格遵守)
1.  **逐点对决**: 你必须严格按照JD中列出的每一条任职要求，逐一进行分析。JD中有7条要求，你的报告中就必须有7个分析回合(rounds)。不得合并、省略或自行总结。
2.  **硬性要求优先**: 必须特别注意JD中任何以 "硬性要求：" 开头的条款。如果候选人不满足任何一条硬性要求，最终匹配度分数【绝不能超过50分】，并且这必须是你最终结论的核心依据。
3.  **层级评估**: 基于所有分析，对候选人进行层级评估，必须从提供的选项中选择一个。
4.  **强制JSON数组输出**: 你的最终输出必须是且仅是一个JSON数组，不包含任何面试建议。
...
# 你的输出格式 (严格遵循此JSON结构，为JD中的每一条要求创建一个round):
```json
[
  {{
    ...
    "analysis_report": {{
      ...
      "final_verdict": {{
        ...
        "verdict_summary": "一句话总结这位候选人是否值得推荐，以及核心理由（如果不满足硬性要求，必须在此处明确指出）。"
      }},
      "rounds": [
        {{
          "round_name": "【第1条要求】: [请从JD中原文复制第一条任职要求]",
          "score": 0, "max_score": 10,
          "analysis": {{
            "strengths": "【基于简历，分析候选人在这项要求上的匹配点和证据】",
            "gaps": "【基于简历，分析候选人在这项要求上的缺失点或风险】"
          }}
        }}
      ],
...
"""
# ### --- END: JOB MATCHING LOGIC CONFIGURATION --- ###
...
# ### --- NEW FOR JOB MATCHING --- ###
ACTIVE_JD_DATA = {} # 用于存储加载的职位描述
# ### --- ADDITION ENDS --- ###
...
# ### --- START: JOB MATCHING INITIALIZATION FUNCTIONS --- ###

def create_job_matching_dirs():
    """【【【 这是新加的函数 】】】
    检查并自动创建职位匹配功能所需的所有文件夹。
    """
    print(">> [System] Creating directories for Job Matching workflow...")
    try:
        # 使用 .mkdir(parents=True, exist_ok=True)
        # parents=True:  如果父目录不存在，会自动创建 (例如 .ai_ecosystem_ipc)
        # exist_ok=True: 如果文件夹已经存在，不会报错
        INBOX_DIR.mkdir(parents=True, exist_ok=True)
        ARCHIVE_DIR.mkdir(parents=True, exist_ok=True)
        JDS_FOLDER_PATH.mkdir(parents=True, exist_ok=True)
        print("   -> Inbox, Archive, and JDs_library folders are ready.")
    except Exception as e:
        print(f"❌ {Colors.RED}[Error] Failed to create directories: {e}{Colors.ENDC}")

# ... (您之前添加的其他 setup_... 函数) ...

# ### --- END: JOB MATCHING INITIALIZATION FUNCTIONS --- ###
...
def setup_job_matching_database():
    """为职位匹配结果初始化专属的SQLite数据库。"""
    try:
        with job_matching_db_lock:
            conn = sqlite3.connect(JOB_MATCHING_DB, check_same_thread=False)
            cursor = conn.cursor()
            cursor.execute(
                f"""
                CREATE TABLE IF NOT EXISTS {JOB_MATCHING_TABLE_NAME} (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    candidate_name TEXT,
                    final_score TEXT,
                    full_ai_report_json TEXT,
            ...
            conn.commit()
            conn.close()
        print(f"✅ {Colors.GREEN}[SQLite] Job Matching DB initialized.{Colors.ENDC}")
        return True
    except Exception as e:
        print(f"❌ {Colors.RED}[SQLite Error] Job Matching DB setup failed: {e}{Colors.ENDC}")
        return False

def add_job_matching_qdrant_collection():
    """在Qdrant中确保职位匹配的集合存在。"""
    try:
        collections = [c.name for c in QDRANT_CLIENT.get_collections().collections]
        if QDRANT_JOB_REPORTS_COLLECTION not in collections:
            QDRANT_CLIENT.create_collection(
                collection_name=QDRANT_JOB_REPORTS_COLLECTION,
                vectors_config=models.VectorParams(
                    size=VECTOR_DIMENSION, distance=models.Distance.COSINE
                ),
            )
            print(f"   -> [Qdrant] Collection '{QDRANT_JOB_REPORTS_COLLECTION}' created.")
    except Exception as e:
        print(f"❌ {Colors.RED}[Qdrant Error] Failed to create job matching collection: {e}{Colors.ENDC}")

def load_jds():
    """加载所有JD到内存中，并忽略带'-'前缀的文件，逻辑和原来完全一样。"""
    global ACTIVE_JD_DATA
    print(">> [System] Loading Job Descriptions (JDs)...")
    
    # 【核心逻辑】筛选出所有不以'-'开头的 .txt 文件
    active_jds = [p for p in JDS_FOLDER_PATH.glob("*.txt") if not p.name.startswith("-")]
    
    for jd_path in active_jds:
        try:
            content = jd_path.read_text("utf-8").strip()
            if content:
                ACTIVE_JD_DATA[jd_path.stem] = {"content": content}
        except Exception as e:
            print(f"   -> {Colors.YELLOW}Warning: Failed to load JD '{jd_path.name}': {e}{Colors.ENDC}")
    
    print(f"✅ {Colors.GREEN}[System] Loaded {len(ACTIVE_JD_DATA)} active JDs.{Colors.ENDC}")
...
def read_resume_file_content(file_path):
    """一个简单的文件读取函数，支持txt和pdf。"""
    p = Path(file_path)
    content = ""
    try:
        # 添加对更多格式的支持，例如 .md 和 .docx
        if p.suffix.lower() in [".txt", ".md"]:
            content = p.read_text("utf-8")
        elif p.suffix.lower() == ".pdf":
            with fitz.open(p) as doc:
                content = "".join(page.get_text() for page in doc)
        # 您可以根据需要添加对 .docx 等格式的支持
        else:
             print(f"   -> {Colors.YELLOW}Unsupported file type: {p.name}{Colors.ENDC}")
             return None
        return content.strip()
    except Exception as e:
        print(f"   -> {Colors.RED}Error reading file {p.name}: {e}{Colors.ENDC}")
        return None

# ### --- START: JOB MATCHING CORE WORKFLOW --- ###
# ... (你已有的 import 和 read_resume_file_content 函数) ...

def render_job_report_to_console(report_json):
    """【【【 这是新加的函数 】】】
    将AI返回的JSON分析报告，格式化成美观的彩色文本并打印到控制台。
    """
    try:
        ...
        print(f"  - {Colors.BOLD}最终匹配度: {score_color}{score}%{Colors.ENDC}")
        print(f"  - {Colors.BOLD}一句话总结: {verdict.get('verdict_summary', 'N/A')}")
        
        for round_data in report.get("rounds", []):
            print(f"\n  --- {round_data.get('round_name', '未知回合')} ---")
            analysis = round_data.get("analysis", {})
            print(f"    {Colors.GREEN}✅ 匹配点:{Colors.ENDC} {analysis.get('strengths', 'N/A')}")
            print(f"    {Colors.YELLOW}⚠️ 风险点:{Colors.ENDC} {analysis.get('gaps', 'N/A')}")
        
        assessment = report.get("talent_tier_assessment", {})
        print(f"\n  - {Colors.BOLD}人才层级评估: {Colors.MAGENTA}{assessment.get('tier', 'N/A')}{Colors.ENDC}")
        print(f"  - {Colors.BOLD}判断依据: {assessment.get('justification', 'N/A')}")

        print(f"{Colors.CYAN}{'=' * 70}{Colors.ENDC}\n")
    except Exception as e:
        print(f"   -> {Colors.RED}Error rendering report to console: {e}{Colors.ENDC}")


def run_smart_analysis_batch(batch_tasks):
    """
    接收一批简历任务，调用AI进行批量分析，然后为每个结果启动后台保存。
    """
    if not batch_tasks or not ACTIVE_JD_DATA:
        return

    print(f"\n{Colors.MAGENTA}>> [Job Analysis] Starting batch analysis for {len(batch_tasks)} resumes...{Colors.ENDC}")

    jd_input_text = "\n\n".join(
        [f"<jd title=\"{t}\">\n{d['content']}\n</jd>" for t, d in ACTIVE_JD_DATA.items()]
    )
    resumes_input_parts = [
        f"<candidate id='{task['id']}'>\n{task['resume_text']}\n</candidate>" for task in batch_tasks
    ]
    resumes_input_text = "\n\n---\n\n".join(resumes_input_parts)
    analysis_prompt = PROMPT_ANALYSIS_REPORTER.format(
        jd_input=jd_input_text, resumes_input=resumes_input_text
    )

    response_text = run_ai_task(analysis_prompt)
    if not response_text:
        print(f"   -> {Colors.RED}AI did not return a response for the batch.{Colors.ENDC}")
        return

    try:
        json_match = re.search(r"```json\s*([\s\S]*?)\s*```", response_text)
        if not json_match:
             raise json.JSONDecodeError("No JSON block found in AI response.", response_text, 0)
        all_analyses = json.loads(json_match.group(1))
    except Exception as e:
        print(f"   -> {Colors.RED}Failed to parse AI response: {e}{Colors.ENDC}")
        return

    task_map = {str(task["id"]): task for task in batch_tasks}
    for analysis_json in all_analyses:
        candidate_id = str(analysis_json.get("candidate_id", ""))
        original_task = task_map.get(candidate_id)
        if original_task:
        # 【【【 在这里添加调用！！！ 】】】
            render_job_report_to_console(analysis_json) # 打印报告到命令行
            # 【【【 添加结束 】】】
            report = analysis_json.get("analysis_report", {})
            name = report.get("title", f"Resume_{candidate_id}").replace("人才价值深度分析报告: ","")
            print(f"   -> {Colors.GREEN}Analysis complete for: {name}. Queueing background save...{Colors.ENDC}")
            background_task_executor.submit(background_save_job_report, analysis_json, original_task['resume_text'])
            background_task_executor.submit(background_vectorize_resume, analysis_json, original_task['resume_text'])

def background_save_job_report(report_json, resume_text):
    """后台任务：将AI分析报告存入SQLite。"""
    try:
        analysis = report_json.get("analysis_report", {})
        name = analysis.get("title", "Unknown").replace("人才价值深度分析报告: ", "")
        score = analysis.get("final_verdict", {}).get("final_match_score_percent", "0")
        
        with job_matching_db_lock:
            conn = sqlite3.connect(JOB_MATCHING_DB, check_same_thread=False)
            cursor = conn.cursor()
            cursor.execute(
                f"INSERT INTO {JOB_MATCHING_TABLE_NAME} (candidate_name, final_score, full_ai_report_json, raw_resume_text, timestamp) VALUES (?, ?, ?, ?, ?)",
                (name, score, json.dumps(report_json, ensure_ascii=False), resume_text, get_local_time_str())
            )
            conn.commit()
            conn.close()
        print(f"✅ {Colors.GREEN}[Async DB] Saved report for '{name}'.{Colors.ENDC}")
    except Exception as e:
        print(f"❌ {Colors.RED}[Async DB Error] Failed to save job report: {e}{Colors.ENDC}")

def background_vectorize_resume(report_json, resume_text):
    """后台任务：将简历向量化并存入Qdrant。"""
    try:
        analysis = report_json.get("analysis_report", {})
        name = analysis.get("title", "Unknown").replace("人才价值深度分析报告: ", "")
        score = float(analysis.get("final_verdict", {}).get("final_match_score_percent", 0))
        vector = generate_text_vector(resume_text)
        ...
        QDRANT_CLIENT.upsert(
            ...
            points=[models.PointStruct(
                ...
                payload={"candidate_name": name, "score": score, "resume_snippet": resume_text[:500]}
            )],
            wait=True
        )
        print(f"✅ {Colors.GREEN}[Async Qdrant] Vectorized resume for '{name}'.{Colors.ENDC}")
    except Exception as e:
        print(f"❌ {Colors.RED}[Async Qdrant Error] Failed to vectorize resume: {e}{Colors.ENDC}")




# ### --- END: JOB MATCHING CORE WORKFLOW --- ###
...
# ==============================================================================
#      请用这个【已修复的版本】，完整替换掉您脚本中旧的 translate_and_read_aloud 函数
# ==============================================================================
...
def translate_and_read_aloud(text, target_lang):
    ...
    (Version 3: Includes a fix for single-word/short-phrase translation).
    """
    if not app_controller_lock.acquire(blocking=False):
    ...
    try:
        ...
        # 1. 清理并检查文本长度
        text_to_translate = clean_text(text)
        # 定义一个规则来判断是否为“短文本”（例如：单词数小于等于3个，且总长度小于25个字符）
        is_short_text = len(text_to_translate.split()) <= 3 and len(text_to_translate) < 25

        # 2. 根据文本长度，选择不同的指令 (Prompt)
        if is_short_text:
            # 如果是短文本，使用一个更直接、更简单的“查字典”指令
            print(f"\n{Colors.BLUE}{action_log} Translating short phrase to {target_lang}... (Simple Mode){Colors.ENDC}")
            if target_lang == "en":
                prompt = f"Please provide the most common English translation for the following word or phrase:\n\n{text_to_translate}"
            else: # target_lang == 'zh'
                prompt = f"请提供以下单词或短语最常用的中文翻译：\n\n{text_to_translate}"
        else:
            # 如果是长文本，则沿用您原来那个功能强大的、用于“文章翻译”的专业指令
            if target_lang == "en":
                print(f"\n{Colors.BLUE}{action_log} Translating to English... (Advanced Mode){Colors.ENDC}")
                ...
                print(f"\n{Colors.BLUE}{action_log} Translating to Chinese... (Advanced Mode){Colors.ENDC}")
...
# ### --- START: JOB MATCHING MONITOR THREAD --- ###

# 在这个函数里，确保顶部有 time 模块的导入
import time

def resume_monitoring_workflow():
    """
    一个在后台独立运行的线程，完全复制了您之前程序的“监控-处理-归档”循环。
    【【【 V2 - 增加了文件写入延时逻辑 】】】
    """
    import shutil 
    
    print(">> [Job Monitor] Resume monitoring thread started.")
    print(f"   -> Watching for new resumes in: {Colors.CYAN}{INBOX_DIR}{Colors.ENDC}")
    
    while True:
        try:
            # 只查找文件，避免处理文件夹等
            inbox_files = [p for p in INBOX_DIR.iterdir() if p.is_file()]

            if inbox_files:
                # 【【【 这就是您要的延时逻辑 】】】
                # 检测到文件后，先等待0.5秒
                print(f"\n{Colors.GREEN}>> [Job Monitor] Detected {len(inbox_files)} new item(s). Waiting for file write to complete...{Colors.ENDC}")
                time.sleep(0.5) 
                # 【【【 延时结束 】】】

                batch_tasks = []
                
                # 重新获取文件列表，以防在等待期间文件被删除
                current_files = [p for p in INBOX_DIR.iterdir() if p.is_file()]

                for file_path in current_files:
                    resume_text = read_resume_file_content(file_path)
                    if resume_text:
                        batch_tasks.append({
                            "id": str(uuid.uuid4()),
                            "path": file_path,
                    ...
                    try:
                        ...
                        print(f"   -> [Archive] Moved '{file_path.name}' to archive.")
                    except Exception as e:
                        # 如果文件已被移动，这里会报错，是正常现象，所以我们静默处理
                        pass

                if batch_tasks:
                    run_smart_analysis_batch(batch_tasks)
                
                print(f">> [Job Monitor] Batch complete. Resuming watch...")

            time.sleep(5)
            
        except Exception as e:
            print(f"❌ {Colors.RED}[Job Monitor Error] An error occurred in monitoring loop: {e}{Colors.ENDC}")
            traceback.print_exc()
            time.sleep(15)
            
# ### --- END: JOB MATCHING MONITOR THREAD --- ###
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("      Bilingual Dialogue Turn Builder v7.5 - with Job Matching Monitor")
    print("=" * 70)
    ...
    if not all([WATCHDOG_AVAILABLE, VECTOR_DB_AVAILABLE]):
        ...
        sys.exit(1)
 # 【【【 将创建文件夹的调用放在所有数据库和服务启动之前 】】】
    create_job_matching_dirs() 
    # ### --- END: CALL JOB MATCHING SETUP --- ###
    # --- 启动所有服务 ---
    if not setup_api(): sys.exit(1)
    if not setup_voice_transcripts_database(): sys.exit(1)
    if not setup_whisper_model(): sys.exit(1)
    if not setup_corpus_database(): sys.exit(1)
    if not setup_daily_records_database(): sys.exit(1)
    if not setup_risk_assessment_database(): sys.exit(1)
    if not setup_concise_qa_database(): sys.exit(1)

    # ### --- START: CALL JOB MATCHING SETUP --- ###
    create_job_matching_dirs() # 创建文件夹
    if not setup_job_matching_database(): sys.exit(1) # 创建数据库
    # ### --- END: CALL JOB MATCHING SETUP --- ###

    setup_vector_database() # 这个函数内部已经被我们修改过来创建新集合
    add_job_matching_qdrant_collection() # 确保调用
    ...
    # ### --- START: LAUNCH THE NEW MONITOR THREAD --- ###
    # 【【【 这是最关键的一步，启动我们的独立监控线程 】】】
    threading.Thread(target=resume_monitoring_workflow, daemon=True).start()
    # ### --- END: LAUNCH THE NEW MONITOR THREAD --- ###

    print("✅ [System] All background services are running.")
