# ==============================================================================
#      Bilingual Dialogue Turn Builder v6.3 - Self-Maintaining
# ==============================================================================
# Version Notes:
# - [【【【 V6.3 Standalone Translation 】】】
#   - Added a new hotkey (Alt+E) for on-the-fly translation.
#   - Automatically detects Chinese -> English or English -> Chinese.
#   - Copies the result to the clipboard and prints it with color.
#   - This action does NOT interact with the database.
...
# --- ANSI color codes ---
class Colors:
    CYAN, GREEN, YELLOW, RED, ENDC, MAGENTA = (
        "\033[96m",
        "\033[92m",
        "\033[93m",
        "\033[91m",
...
TRIGGER_FILES = {
    ...
    "standalone_translate": IPC_DIR / "trigger_standalone_translate",
}
...
# --- [NEW] Standalone Translation Function ---
def standalone_translate(text_to_process):
    """
    Detects the language of the input text and translates it to the other language.
    ...
    try:
        print(
            f"\n[Standalone Translate] Received text: “{text_to_process[:50].strip()}...”"
        )

        # 1. Detect language
        if is_primarily_chinese(text_to_process):
            target_lang, source_name, target_name, color = (
                "en",
                "Chinese",
                "English",
                Colors.CYAN,
        ...
        print(f">> Detected {source_name}, translating to {target_name}...")

        # 2. Translate
        translated_text = intelligent_translate(text_to_process, target_lang)

        # 3. Copy to clipboard
        pyperclip.copy(translated_text)

        # 4. Display highlighted output
        print(
            f"{Colors.MAGENTA}--- [ Standalone Translation ({target_name}) | Copied ] ---{Colors.ENDC}"
        )
        print(f"{color}{translated_text}{Colors.ENDC}")
        print(f"{Colors.MAGENTA}{'-'*52}{Colors.ENDC}")

        safe_notification(f"Translated to {target_name}", "Result copied to clipboard.")

    except Exception as e:
        safe_notification("Standalone Translation Failed", str(e))
        traceback.print_exc()
    finally:
        with app_controller.lock:
            app_controller.is_processing = False
...
def is_primarily_chinese(text):
    """
    Checks if the text contains Chinese characters. A simple but effective check.
    """
...
class FileTriggerHandler(FileSystemEventHandler):
    def __init__(self):
        self.function_map = {
            ...
            str(TRIGGER_FILES["standalone_translate"]): create_hotkey_handler(
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("      Bilingual Dialogue Turn Builder v6.3 - Self-Maintaining")
    print("=" * 70)
    ...
    print(
        f"  4. One-off translation? Press [Alt+E] -> {Colors.MAGENTA}Translates without saving.{Colors.ENDC}"
    )
    ...
    print(
        f"  - {Colors.MAGENTA}Standalone Translate (Alt+E): {TRIGGER_FILES['standalone_translate']}{Colors.ENDC}"
    )
