...
def cleanup_duplicates():
    ...
    try:
        with db_lock:
            conn = sqlite3.connect(CORPUS_DB, check_same_thread=False)
            cursor = conn.cursor()
            # This query finds all `input_text` that are duplicated, and for each group of duplicates,
            # it selects the IDs of all rows except for the one with the smallest ID (the first one saved).
            query = f"""
...
if __name__ == "__main__":
    ...
    cleanup_duplicates()  # Run new deduplication function on startup
    cleanup_old_orphans()
    # ### --- MODIFIED BLOCK END --- ###
