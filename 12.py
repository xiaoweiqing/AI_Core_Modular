...
def translate_to_english(text):
    ...
    try:
        ...
        threading.Thread(
            target=save_log_to_notion,
            args=("翻译 (英)", text, translated_text),
            daemon=True,
        ).start()
        safe_notification("AI 翻译 (英)", "结果已复制到剪贴板！")

    except Exception as e:
        print(f"!! [翻译->英] 调用API时出错: {e}")
...
def translate_to_chinese(text):
    ...
    try:
        ...
        threading.Thread(
            target=save_log_to_notion,
            args=("翻译 (中)", text, translated_text),
            daemon=True,
        ).start()
        safe_notification("AI 翻译 (中)", "翻译结果已复制！")

    except Exception as e:
        print(f"!! [翻译->中] 调用API时出错: {e}")
        safe_notification("翻译失败", str(e))
    finally:
        # 释放锁
        with app_controller.lock:
