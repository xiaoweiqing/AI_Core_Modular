...
# (Add this new function in Section 5, alongside optimize_prompt_for_ai and stateless_translate)
### --- NEW OFFLINE TTS FUNCTION --- ###
### --- NEW OFFLINE TTS FUNCTION (V2 - Handles Long Text) --- ###
### --- NEW OFFLINE TTS FUNCTION (V3 - Robust Error Handling & Timeout) --- ###
def read_text_aloud(text):
    """
    Uses the local Piper TTS engine. This version has robust error handling,
    a timeout for safety, and will report any errors from the Piper process.
    """
    if not app_controller_lock.acquire(blocking=False):
    ...
    try:
        print(
            f"\n{Colors.BLUE}[Offline Read Aloud] Generating audio with Piper...{Colors.ENDC}"
        )
        ...
        # 1. Create and write to a temporary text file
        with tempfile.NamedTemporaryFile(
            mode="w+", suffix=".txt", delete=False, encoding="utf-8"
        ) as fp_text:
            temp_text_file = fp_text.name
            fp_text.write(text)
        print(f"   -> Text saved to temporary file: {temp_text_file}")

        # 2. Create a temporary audio file path
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as fp_audio:
            temp_audio_file = fp_audio.name

        # 3. Build and run the Piper command with a timeout
        piper_command = [
            str(piper_exe),
            "--model",
            str(model_file),
            "--config",
            str(model_file) + ".json",  # Explicitly add config
            "--output_file",
            temp_audio_file,
        ]

        # Read from stdin, which is more robust than --data-path for piper
        print("   -> Running Piper command...")
        process = subprocess.run(
            piper_command,
            input=text,  # Feed text via stdin pipe
            text=True,
            encoding="utf-8",
            check=True,
        ...
        print("   -> Piper finished successfully. Now playing audio...")

        # 4. Play the generated audio
        subprocess.run(["aplay", temp_audio_file], check=True, capture_output=True)
        print(f"   -> {Colors.GREEN}Playback complete.{Colors.ENDC}")

    except subprocess.TimeoutExpired:
        error_message = "The TTS process took too long and was stopped."
        print(f"   -> {Colors.RED}{error_message}{Colors.ENDC}")
        safe_notification("TTS Error", error_message)
    except subprocess.CalledProcessError as e:
        # THIS IS THE CRITICAL PART - IT WILL SHOW US PIPER'S ERROR
        error_message = f"Piper failed with an error:\n{e.stderr}"
        print(f"   -> {Colors.RED}{error_message}{Colors.ENDC}")
        safe_notification("Piper TTS Error", "Piper failed. Check the console.")
    except Exception as e:
        error_message = f"An unexpected error occurred: {e}"
        print(f"   -> {Colors.RED}{error_message}{Colors.ENDC}")
        safe_notification("TTS Error", "An unexpected error occurred.")
    finally:
