...
if __name__ == "__main__":
    ...
    print("=" * 70)
    # ### --- MODIFIED --- ###
    print("      Bilingual Dialogue Turn Builder v7.4 - Daily Logger Update")
    print("=" * 70)
    ...
    ### --- NEW --- ###
    if not setup_daily_records_database():  # Initialize the new database
        sys.exit(1)
    if not setup_risk_assessment_database():  # <-- ADD THIS
        sys.exit(1)
    ...
    print(f"  {Colors.GREEN}✨ NEW: Daily activity logging is now active!{Colors.ENDC}")
    print("\n  WORKFLOW:")
    print(
        f"  {Colors.BLUE}--- AI Power Tools (Now with Auto-Logging!) ---{Colors.ENDC}"
    )
    print(
        f"  1. Select text, press [Alt+Q] -> Translates to {Colors.CYAN}English{Colors.ENDC}"
    )
    print(
        f"  2. Select text, press [Alt+W] -> Translates to {Colors.GREEN}Chinese{Colors.ENDC}"
    )
    print(
    ...
    print(
        f"  4. Select [Input],  press [Alt+S] -> {Colors.MAGENTA}Saves Input{Colors.ENDC} to corpus."
    )
    print(
