...
TRIGGER_FILES = {
    ...
    "export_context": IPC_DIR
    / "trigger_export_context",  # ### --- ADD THIS LINE --- ### Alt+G
...
def export_recent_context():
    ...
    try:
        ...
        # 1. Fetch data from the daily records database
        with daily_db_lock:
            conn = sqlite3.connect(DAILY_RECORDS_DB, check_same_thread=False)
            cursor = conn.cursor()
            cursor.execute(
        ...
        # 3. Define the output file path and ensure the directory exists
...
class FileTriggerHandler(FileSystemEventHandler):
    def __init__(self):
        self.function_map = {
            ...
            str(
                TRIGGER_FILES["export_context"]
            ): export_recent_context,  # ### --- ADD THIS LINE --- ###
