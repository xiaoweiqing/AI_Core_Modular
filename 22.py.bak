# ==============================================================================
#      Bilingual Dialogue Turn Builder v7.2 - Feature Update
# ==============================================================================
# Version Notes:
...
#     (AI calls, DB writes, Vectorization) to ensure system resilience.
#   - ADDED: Startup health check for the Qdrant vector database connection.
#   - IMPROVED: Significantly more detailed logging for all operations, making
#     the system's status clear at all times.
#   - HARDENED: Made the startup cleanup function more robust against malformed data.
# - [【【【 V7.0 The Perfect Version - Final Design 】】】
#   - Full separation of Translation (Alt+Q/W) and Data Recording (Alt+S/D).
# ==============================================================================
import os
...
TRIGGER_FILES = {
    ...
    "mark_high_quality": IPC_DIR / "trigger_mark_high_quality",  # Alt+F <-- 新增功能
}
...
last_record_id = None
last_completed_id = None  # <-- 新增：用于跟踪最后一个完成的ID
...
# A robust lock for controlling primary user-facing operations
...
# --- 5. Stateless Translation Workflow (Alt+Q, Alt+W) ---
...
def save_output(text):
    global last_record_id, last_completed_id  # <-- 修改此行
    with app_controller_lock:
        ...
        last_completed_id = last_record_id  # <-- 新增此行：记录刚刚完成的ID
        last_record_id = None
        threading.Thread(
            target=process_metadata_and_vectorize,
...
def mark_as_high_quality():
    """Marks the most recently completed record as 'high-quality'."""
    ...
    with app_controller_lock:
        ...
        # Reset to prevent accidental re-marking of the same record
...
def create_hotkey_handler(target_func, *args):
    def handler():
        # 这个特殊的 if 判断在您的原始代码中不存在，所以为了严格遵守您的代码，我把它移除了。
        # cancel_last_turn 和 mark_as_high_quality 现在直接从 FileTriggerHandler 调用。
        def task_with_selected_text():
            text_to_process = ""
            # 【【【 核心修正区域 START 】】】
            # 1. 优先尝试获取鼠标选中的文本 (Primary Selection)
            try:
                # 适用于 X11 (大多数桌面环境)
                ...
                try:
                    ...
                    text_to_process = ""  # 确保为空，以便进行下一步

            # 2. 如果鼠标没有选中任何内容，再尝试获取剪贴板 (Clipboard)
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("      Bilingual Dialogue Turn Builder v7.2 - Feature Update")
    print("=" * 70)
    ...
    print("\n  WORKFLOW:")
    print(f"  {Colors.BLUE}--- Temporary Translation ---{Colors.ENDC}")
    print(
    ...
    print(
        f"  3. Select [Input],  press [Alt+S] -> {Colors.MAGENTA}Saves Input{Colors.ENDC} to database."
    )
    print(
        f"  4. Select [Output], press [Alt+D] -> {Colors.MAGENTA}Saves Output{Colors.ENDC}, completes pair."
    )
    print(
        f"  5. Mistake?      Press [Alt+C] -> {Colors.YELLOW}Cancels{Colors.ENDC} pending input."
    )
    # --- 新增行 START ---
    print(
        f"  6. After pair,   press [Alt+F] -> Marks as {Colors.BLUE}High-Quality{Colors.ENDC}."
    )
    # --- 新增行 END ---
