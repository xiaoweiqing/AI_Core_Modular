# ==============================================================================
#      AI Ecosystem Core v16.3 - 翻译增强版
# ==============================================================================
# 版本说明:
# - 【【【 V16.3 翻译功能增强 】】】
#   - 翻译模块升级: 将原有的“自动识别语言翻译”功能，拆分为两个更精准、
#     更高效的独立功能：“强制翻译为英文”和“强制翻译为中文”。
#   - 热键拆分: 为两个新的翻译功能分配了独立的信号文件，允许用户在系统
#     中设置不同的快捷键，实现“一键英译”或“一键中译”。
# - 【 V16.0 架构升级 - 原生、精准、高效 】
#   - 核心数据库: 从 ChromaDB 迁移到性能更强、更专业的 Qdrant 向量数据库。
#   - 原生化集成: 全面采用 Linux 原生命令行工具(xclip, gnome-screenshot, notify-send)。
#   - 交互模式变革: 用户通过精确热键调用功能模式。
# ==============================================================================
# ==============================================================================
#      【【【 v16.1 终极网络修复模块 】】】
# ==============================================================================
...
import uuid
from langchain_openai import ChatOpenAI # <<< 【【【 在这里添加这一行 】】】
...
TRIGGER_FILES = {
    ...
    "translate_to_en": IPC_DIR / "trigger_translate_to_en",  # 新增：翻译到英文
    "translate_to_zh": IPC_DIR / "trigger_translate_to_zh",  # 新增：翻译到中文
...
class FileTriggerHandler(FileSystemEventHandler):
    def __init__(self):
        # 将信号文件名映射到对应的处理函数
        # 【【【 修改点 2/4：更新 function_map 映射 】】】
        self.function_map = {
            str(TRIGGER_FILES["rag_qa"]): create_hotkey_handler(
            ...
            str(TRIGGER_FILES["optimize_post"]): create_hotkey_handler(
                optimize_social_post
            ),
            ...
            str(TRIGGER_FILES["optimize_comm"]): create_hotkey_handler(
                optimize_communication
            ),
            str(TRIGGER_FILES["quick_save"]): create_hotkey_handler(
                quick_save_to_inbox
            ),
            str(TRIGGER_FILES["process_jd"]): create_hotkey_handler(
                process_clipboard_jd
            ),
...
def safe_notification(title, message, **kwargs):
    ...
    try:
        subprocess.run(
            ["notify-send", title, message, "-a", "AI Ecosystem Core", "-t", "4000"],
            check=True,
        )
    ...
    def handler():
        ...
        def task_with_selected_text():
            ...
            if text_to_process and not text_to_process.isspace():
                # 使用线程处理，避免阻塞UI
                threading.Thread(
                    target=target_func, args=(text_to_process,), daemon=True
                ).start()
            else:
                print(">> [热键处理] 未获取到有效文本。")
                safe_notification("未找到文本", "请先用鼠标选中或复制文本")
...
def google_search_for_context(query, num_results=3):
    """【Google 搜索集成模块】"""
    if (
        not GOOGLE_SEARCH_API_KEY
        or "..." in GOOGLE_SEARCH_API_KEY
        or not GOOGLE_SEARCH_CX
    ):
        return ""
    ...
    try:
        ...
        res = (
            service.cse().list(q=query, cx=GOOGLE_SEARCH_CX, num=num_results).execute()
        )
        if "items" not in res:
            return ""
        search_results = [
            f"【网络来源 {i+1}: {item.get('title', '无标题')}】\n摘要: {item.get('snippet', '无摘要').replace(chr(10), ' ')}\n链接: {item.get('link', '#')}"
        ...
        if "quota" in str(e).lower():
            print("   -> 提示：可能是每日免费搜索配额(100次)已用完。")
        return ""


# --- 2. API 初始化 ---
# 【【【 修改点：使用 llm_client 替换 gemini_model 】】】
llm_client = None 

def setup_api():
    """
    【本地化修改】初始化，连接到本地的 LLM 服务。
    """
    global llm_client
    try:
        # 检查关键环境变量，即使是本地模型，也保持原有结构的完整性
        if not API_KEY or not NOTION_TOKEN:
             print("!! 警告：虽然模型已本地化，但 .env 文件中的关键 Key (如 Notion) 仍需配置。")
        
        print(f">> [AI] 正在连接到本地模型 API at [http://127.0.0.1:8087/v1]...")
        
        # 参考您的 MuseBox 配置来创建客户端
        llm_client = ChatOpenAI(
            openai_api_base="http://127.0.0.1:8087/v1",
            openai_api_key="not-needed",  # 本地模型通常不需要key
            model_name="local-model", # 模型名称可以任意填写
            temperature=0.1,         # 翻译任务建议使用较低的温度
            request_timeout=90,      # 设置请求超时时间
        )
        
        # 发送一个简短的测试请求来验证连接
        llm_client.invoke("Hello")
        print(f"✅ [AI] 本地大语言模型连接成功。")
        return True
    except Exception as e:
        print(f"❌ [AI] 严重错误: 连接本地模型失败: {e}")
        print("   -> 请确保您的本地AI模型服务 (如Ollama + LiteLLM) 正在运行，并监听 8087 端口。")
        return False


# --- 3. 写入AI训练中心 ---
def write_to_training_hub(
    task_type, input_text, output_text, source_db_name, source_page_id
):
    if not TRAINING_HUB_DATABASE_ID or len(TRAINING_HUB_DATABASE_ID) < 32:
        return
    try:
        ...
        properties_data = {
            ...
            "理想输出 (Output)": {
                "rich_text": [{"text": {"content": safe_output_text}}]
            },
            "标注状态": {"select": {"name": "待审核"}},
        }
        relation_column_map = {
            "CoreBrain": RELATION_LINK_BRAIN,
            "Log": RELATION_LINK_LOG,
            "Candidate": RELATION_LINK_CANDIDATE,
        }
        if source_db_name in relation_column_map and source_page_id:
            properties_data[relation_column_map[source_db_name]] = {
                "relation": [{"id": source_page_id}]
            }
        notion.pages.create(
            parent={"database_id": TRAINING_HUB_DATABASE_ID}, properties=properties_data
        )
...
def add_knowledge_to_qdrant(collection_name, document_id, document_text, metadata):
    ...
    try:
        print(
            f">> [Qdrant] 正在向量化知识: “{metadata.get('source_title', 'Untitled')[:30]}...”"
        )
        ...
        print(
            f"✅ [Qdrant] 知识已成功添加/更新到 '{collection_name}'！当前总量: {current_count}"
        )
...
def save_log_to_notion(log_type: str, input_text: str, output_text: str):
    if len(TOOLBOX_LOG_DATABASE_ID) < 32:
        return
    try:
        notion = notion_client.Client(auth=NOTION_TOKEN)
        ...
        children_blocks = [
            ...
            for chunk in [
                output_text[i : i + 1900] for i in range(0, len(output_text), 1900)
            ]
        ]
        ...
        properties_data = {
            ...
            "输入内容": {"rich_text": [{"text": {"content": input_text[:1900]}}]},
            "输出摘要": {"rich_text": [{"text": {"content": output_text[:1900]}}]},
            "记录日期": {"date": {"start": current_date_iso}},
        }
        new_log_page = notion.pages.create(
            parent={"database_id": TOOLBOX_LOG_DATABASE_ID},
        ...
        threading.Thread(
            ...
            args=(
                task_type_map.get(log_type, "通用任务"),
                input_text,
                output_text,
                "Log",
                log_page_id,
        ...
        metadata = {
            ...
            "original_id": log_page_id,
        }
        threading.Thread(
            target=add_knowledge_to_qdrant,
            args=("ai_ecosystem_core", log_page_id, knowledge_text, metadata),
            daemon=True,
        ).start()
    except Exception as e:
    ...
    try:
        ...
        children_blocks = (
            [
                ...
                for chunk in [
                    page_content[i : i + 1900]
                    for i in range(0, len(page_content), 1900)
                ]
            ]
            if page_content
            else []
        ...
        current_date_iso = datetime.now(timezone(timedelta(hours=8))).isoformat()
        properties_data = {
            "标题": {"title": [{"text": {"content": page_title}}]},
            "日期": {"date": {"start": current_date_iso}},
        }

        new_inbox_page = notion.pages.create(
        ...
        metadata = {
            ...
            "source_title": page_title,
            "original_id": inbox_page_id,
        }
        threading.Thread(
            target=add_knowledge_to_qdrant,
            args=("ai_ecosystem_core", inbox_page_id, cleaned_text, metadata),
            daemon=True,
        ).start()

        print(">> [功能联动] 已将该内容提交给“核心大脑”进行深度处理和归档...")
        threading.Thread(
            target=process_data_packet, args=(cleaned_text,), daemon=True
        ).start()
...
def answer_question_with_rag(question):
    ...
    try:
        ...
        personal_context = retrieve_from_multiple_collections(
            question, collection_names=collections_to_search, top_n=3
        )

        web_context_future = ThreadPoolExecutor(max_workers=1).submit(
            google_search_for_context, optimized_query
        )
        web_context = web_context_future.result()

        combined_context = ""
        if personal_context:
            combined_context += "--- 【个人知识库信息】 ---\n" + personal_context
        if web_context:
            combined_context += "\n\n--- 【实时网络搜索结果】 ---\n" + web_context
        ...
        source_map = {
            ...
            "google_search": "🌐 实时网络搜索",
            "general_knowledge": "🧠 LLM通用知识",
        }
        sources = ", ".join(
            [source_map.get(s, s) for s in result_data.get("source_used", [])]
        )
        print(f"信息来源: {sources}")
        print(
            f"📈 置信度 (基于上下文): {result_data.get('confidence_score', 0) * 100:.1f}%"
        )
        ...
        threading.Thread(
            target=save_log_to_notion,
            args=("RAG问答", question, full_answer_for_log),
            daemon=True,
        ).start()
...
def quick_ocr_logic(img):
    ...
    try:
        ...
        prompt = (
            "# 指令：请提取并返回这张图片中的所有文字，只返回纯文本，不要任何其他内容。"
        )
        response = gemini_model.generate_content(
            [prompt, img], request_options={"timeout": 30}
        )
...
def trigger_deep_analysis():
    """热键触发器：深度分析"""
    threading.Thread(
        target=run_screenshot_flow,
        args=(analyze_screenshot_deep_logic, "深度分析"),
        daemon=True,
    ).start()
...
def trigger_quick_ocr():
    """热键触发器：极速OCR"""
    threading.Thread(
        target=run_screenshot_flow, args=(quick_ocr_logic, "极速OCR"), daemon=True
    ).start()


# --- 9. 核心大脑与智能关联 (适配v16.0) ---
def save_to_core_brain_db(title, category, summarized_question, answer):
    if len(CORE_BRAIN_DATABASE_ID) < 32:
        return
    try:
        notion = notion_client.Client(auth=NOTION_TOKEN)
        ...
        current_date_iso = datetime.now(timezone(timedelta(hours=8))).isoformat()
        properties_data = {
            "主题": {"title": [{"text": {"content": title}}]},
            "我的问题": {"rich_text": [{"text": {"content": summarized_question}}]},
            "分类": {"select": {"name": category}},
            "记录日期": {"date": {"start": current_date_iso}},
        }
        new_page = notion.pages.create(
            parent={"database_id": CORE_BRAIN_DATABASE_ID},
            properties=properties_data,
            children=children_blocks,
        )
        print(">> [核心大脑] 知识归档成功！")
        ...
        metadata = {
            "source_db": "core_brain",
            "task_type": "knowledge_capture",
            "source_title": title,
            "original_id": new_page_id,
        }
        threading.Thread(
            target=add_knowledge_to_qdrant,
            args=("ai_ecosystem_core", new_page_id, text_to_embed, metadata),
            daemon=True,
        ).start()

        threading.Thread(
            target=write_to_training_hub,
            args=("问答配对", summarized_question, answer, "CoreBrain", new_page_id),
            daemon=True,
        ).start()
    except Exception as e:
...
def process_data_packet(full_text):
    ...
    try:
        ...
        save_to_core_brain_db(
            title=result.get("title", "知识主题生成失败"),
            category=result.get("category", "Resource"),
            summarized_question=result.get("summarized_question", "问题精炼失败"),
            answer=result.get("ai_answer", full_text),
        )
    except Exception as e:
        print(f"!! [核心大脑] 系统处理异常: {e}")
        save_to_core_brain_db(
            title=f"【未处理数据包】{full_text[:30]}...",
            category="Task",
            summarized_question="系统处理失败，请手动整理",
            answer=full_text,
        )
    finally:
        with app_controller.lock:
...
def get_ai_reply(text):
    ...
    try:
        ...
        threading.Thread(
            target=save_log_to_notion, args=("智能回复", text, ai_response), daemon=True
        ).start()
        safe_notification("AI 智能回复", "回复已生成，可直接粘贴！")
    except Exception as e:
        print(f"!! [智能回复] 调用API时出错: {e}")
    finally:
        with app_controller.lock:
            app_controller.is_processing = False
...
def get_ai_analysis(text):
    ...
    try:
        ...
        print(f"\n[文本分析] 正在分析: “{text[:50]}...”")
        response = gemini_model.generate_content(
            prompt, request_options={"timeout": 120}, stream=True
        )
        print("\n--- AI分析中... ---\n")
        ...
        threading.Thread(
            target=save_log_to_notion,
            args=("文本分析", text, full_response_text),
            daemon=True,
        ).start()
    except Exception as e:
        print(f"\n!! [文本分析] 调用API时出错: {e}")
...
def translate_to_english(text):
    ...
    try:
        with app_controller.lock:
            if app_controller.is_processing:
                safe_notification("AI 系统正忙", "请等待上一个任务完成。")
                return
            app_controller.is_processing = True
        
        print(f"\n[本地翻译->英] 正在翻译: “{text[:40].strip()}...”")
        
        # 【【【 核心修改点：调用本地模型 】】】
        response = llm_client.invoke(prompt)
        # 本地模型的返回内容在 .content 属性里
        translated_text = response.content.strip() 
        
        pyperclip.copy(translated_text)
        print(f"[本地翻译->英] 成功 (已复制):\n---\n{translated_text}\n---")
        
        threading.Thread(target=save_log_to_notion, args=("翻译 (英)", text, translated_text), daemon=True).start()
        safe_notification('AI 翻译 (英)', '结果已复制到剪贴板！')
        
    except Exception as e:
        print(f"!! [本地翻译->英] 调用API时出错: {e}")
...
def translate_to_chinese(text):
    ...
    try:
        ...
        pyperclip.copy(translated_text)
        print(f"[本地翻译->中] 成功 (已复制):\n---\n{translated_text}\n---")
        
        threading.Thread(target=save_log_to_notion, args=("翻译 (中)", text, translated_text), daemon=True).start()
        safe_notification('AI 翻译 (中)', '翻译结果已复制！')

    except Exception as e:
        print(f"!! [本地翻译->中] 调用API时出错: {e}")
        safe_notification('翻译失败', str(e))
    finally:
        with app_controller.lock:
            app_controller.is_processing = False
...
def analyze_and_optimize_jd(jd_text):
    ...
    try:
        ...
        print(
            f"   ✅ Gemini处理完成！提炼出的职位名称为: '{result.get('position_title')}'"
        )
        return result
    except Exception as e:
        print(f"   !! 调用Gemini时出错: {e}")
        return None
...
def create_jd_page_in_notion(analysis_result):
    ...
    title = (
        analysis_result.get("position_title")
        or f"剪贴板JD {datetime.now().strftime('%H:%M')}"
    )
    try:
        notion = notion_client.Client(auth=NOTION_TOKEN)
        properties = {
        ...
        children = [
            {
                "type": "heading_2",
                "heading_2": {"rich_text": [{"text": {"content": "原始JD文本"}}]},
            },
            {
                "type": "code",
                "code": {
                    "rich_text": [
                        {
                            "text": {
                                "content": analysis_result.get("original_jd", "")[:1990]
                            }
                        }
                    ],
                    "language": "plain text",
            ...
            {"type": "divider", "divider": {}},
            {
                "type": "heading_2",
                "heading_2": {
                    "rich_text": [{"text": {"content": "🤖 AI招聘顾问分析与建议"}}]
                },
            },
            {
                "type": "paragraph",
                "paragraph": {
                    "rich_text": [
                        {
                            "text": {
                                "content": analysis_result.get("analysis_notes", "")[
                                    :1990
                                ]
                            }
                        }
            ...
            {
                "type": "heading_3",
                "heading_3": {"rich_text": [{"text": {"content": "优化后的JD版本"}}]},
            },
            {
                "type": "code",
                "code": {
                    "rich_text": [
                        {
                            "text": {
                                "content": analysis_result.get("optimized_jd", "")[
                                    :1990
                                ]
                            }
                        }
        ...
        new_page = notion.pages.create(
            parent={"database_id": JD_HUB_DATABASE_ID},
...
def process_clipboard_jd(text):
    ...
    try:
        ...
        if knowledge_text:
            metadata = {
                "source_db": "job_description",
                "task_type": "jd_indexing",
                "source_title": result.get("final_position_title"),
                "original_id": page_id,
            }
            threading.Thread(
                target=add_knowledge_to_qdrant,
                args=("ai_recruitment_assistant", page_id, knowledge_text, metadata),
                daemon=True,
            ).start()
        ...
        safe_notification(
            "JD处理完成", f"职位 '{result.get('final_position_title')}' 已归档"
        )
    finally:
        with app_controller.lock:
            app_controller.is_processing = False
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("  AI Ecosystem Core v16.3 - 翻译增强版")
    print("=" * 70)
    ...
    print(f"  - 翻译成英文:   touch {TRIGGER_FILES['translate_to_en']}")
    print(f"  - 翻译成中文:   touch {TRIGGER_FILES['translate_to_zh']}")
