...
def read_text_aloud(text, is_background_task=False):
    """
    The master function for reading text aloud using the ONLINE gTTS engine.
    It intelligently selects the correct language for the voice.
    It runs asynchronously in a new thread to avoid blocking.
    """

    # This entire function will be run in a background thread.
    def task():
        temp_audio_file = None
        try:
            print(
                f"\n{Colors.BLUE}[Online Read Aloud] Generating audio with gTTS...{Colors.ENDC}"
            )

            # --- 【【【 Core Logic: Select language for gTTS 】】】 ---
            ...
            # Use mpg123, which is perfect for the mp3 files gTTS creates
            subprocess.run(
                ["mpg123", "-q", temp_audio_file], check=True, capture_output=True
            )
    ...
    # --- Start the entire reading task in a new background thread ---
    threading.Thread(target=task, daemon=True).start()


def translate_and_read_aloud(text, target_lang):
    """
    A unified workflow for both Alt+Q and Alt+W using the online Google API.
    It translates and then kicks off the asynchronous reading task.
    """
    ...
    try:
        if target_lang == "en":
            ...
            print(
                "\n"
                + Colors.BLUE
                + "[Translate & Read] Translating to Chinese..."
                + Colors.ENDC
            )
        ...
        print(color + final_text + Colors.ENDC)

        # --- Asynchronously start the reading task ---
        # The main function does NOT wait for it to finish.
        print("   -> Kicking off background TTS...")
        read_text_aloud(final_text)

    finally:
        # The lock is released immediately, not waiting for the audio.
