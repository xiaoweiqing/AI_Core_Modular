...
def safe_notification(title, message):
    ...
    try:
        ...
        subprocess.run(['notify-send', title, message, '-a', 'MuseBox', '-t', '5000'], check=True)
...
async def process_and_update_in_background(db_id, raw_text):
    ...
    await loop.run_in_executor(None, safe_notification, notification_title, notification_message)
