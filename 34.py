...
# ==========================================================
#     Python 脚本修改点 1: C++ 集成 (最终胜利版)
# ==========================================================
# --- [NEW] C++ Bridge Integration ---
CPP_BRIDGE_ENABLED = False  # 默认值，由 __main__ 控制
CPP_BRIDGE_AVAILABLE = False  # 默认值，由 try...except 控制

# 【关键】我们不再在全局作用域尝试导入 my_bridge。
# 因为这需要 LD_LIBRARY_PATH，而这个变量只有在启动脚本运行时才存在。
# 我们只检查 build_dir 是否存在。真正的导入测试将在 setup_api 中进行。

build_dir = Path("/home/weiyubin/projects/cpp_python_bridge/build-final")
if build_dir.exists():
    sys.path.append(str(build_dir))
    # 我们在这里只把路径加进去，不执行 import
    print(f"✅ [System] Found C++ build directory. Path added to sys.path.")
# ==========================================================
...
#     Python 脚本修改点 2: setup_api (最终胜利版)
# ==========================================================
def setup_api():
    """Initializes the AI model, preferring the C++ bridge if enabled and available."""
    global llm, CPP_BRIDGE_AVAILABLE

    if CPP_BRIDGE_ENABLED:
        try:
            # 【关键】真正的 import 测试在这里进行！
            import my_bridge

            CPP_BRIDGE_AVAILABLE = True
            print(
                f"✅✅✅ {Colors.GREEN}[C++ Bridge] Direct C++ bridge is enabled and successfully imported!{Colors.ENDC}"
            )
            return True
        except ImportError as e:
            CPP_BRIDGE_AVAILABLE = False
            print(
                f"❌ {Colors.RED}[C++ Bridge Error] FAILED to import 'my_bridge'.{Colors.ENDC}"
            )
            print(
                f"   {Colors.YELLOW}This means the OS could not find 'libllama.so'. Ensure you are using the 'start.sh' script.{Colors.ENDC}"
            )
            print(f"   Original Error: {e}")
            # fall through to API connection

    # --- 如果 C++ 桥接未启用或导入失败，则执行原始的 API 连接逻辑 ---
    CPP_BRIDGE_ENABLED = False  # 强制关闭开关
    local_api_url = "http://1227.0.0.1:8087/v1"
    try:
        print(
...
# ==========================================================
# ==========================================================
#     Python 脚本修改点 3: run_ai_task (最终胜利版)
# ==========================================================
# (在您的 29.py 中)
def run_ai_task(prompt):
    try:
        ...
        # C++ 内部已经处理了状态重置，所以我们不再需要清理 think 标签等
