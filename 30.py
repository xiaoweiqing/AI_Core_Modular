...
TRIGGER_FILES = {
    ...
    "export_context": IPC_DIR / "trigger_export_context",  # Alt+G (NEW)
...
### --- NEW FUNCTION START --- ###
### --- NEW FUNCTION START --- ###
def export_recent_context():
    """
    Retrieves the last four interactions from the daily records database
    and saves them to the project's dedicated context folder.
    """
    ...
    try:
        print(
            f"\n{Colors.BLUE}[Context Export] Fetching recent interactions...{Colors.ENDC}"
        )

        # 1. Fetch data from the daily records database (This part is unchanged)
        with daily_db_lock:
            conn = sqlite3.connect(DAILY_RECORDS_DB, check_same_thread=False)
            cursor = conn.cursor()
            cursor.execute(
                f"""
                SELECT timestamp, event_type, original_text, processed_text, ai_summary
                FROM {DAILY_RECORDS_TABLE_NAME}
                ORDER BY id DESC
                LIMIT 4
                """
            )
        ...
        if not records:
            safe_notification(
                "Export Failed", "No recent interactions found to export."
            )
            print(f"{Colors.YELLOW}[!] No records found in the daily log.{Colors.ENDC}")
            return

        # 2. Format the data for clarity (This part is unchanged)
        records.reverse()
        context_data = [
            {
                "timestamp": r[0],
        ...
        # 3. Define the NEW output file path and ensure the directory exists
        # ### --- THIS IS THE MODIFIED BLOCK --- ###
        # Set the base path to your project folder from the image
        ...
        export_path = base_path / "session_context.json"

        with open(export_path, "w", encoding="utf-8") as f:
            json.dump(context_data, f, ensure_ascii=False, indent=4)

        # 4. Notify the user with the correct, new path
        final_message = f"Saved last {len(records)} interactions to:\n{export_path}"
        print(f"✅ {Colors.GREEN}[Export Complete] {final_message}{Colors.ENDC}")
        safe_notification("Context Exported", final_message)

    except Exception as e:
        error_message = f"Failed to export context: {e}"
        print(f"❌ {Colors.RED}[Export Error] {error_message}{Colors.ENDC}")
...
class FileTriggerHandler(FileSystemEventHandler):
    ...
    def __init__(self):
        self.function_map = {
            ...
            str(TRIGGER_FILES["export_context"]): export_recent_context,  # NEW
...
if __name__ == "__main__":
    ...
    print(
        f"  ✨ [Alt+G] -> {Colors.BLUE}Exports recent conversation context to a file{Colors.ENDC}"
    )
