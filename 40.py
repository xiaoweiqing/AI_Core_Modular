...
def read_text_aloud(text, is_background_task=False):
    ...
    It now intelligently selects the correct language model (English or Chinese).
    """
    ...
    try:
        ...
        if is_primarily_chinese(text):
            print("   -> Detected Chinese text, using Chinese voice model.")
            model_file = script_dir / "zh_CN-huayan-medium.onnx"
        else:
            print("   -> Detected English text, using English voice model.")
...
def translate_and_read_aloud(text, target_lang):
    """
    A unified workflow for both Alt+Q and Alt+W.
    Translates, copies, and reads aloud as a single, atomic operation.
    """
    if not app_controller_lock.acquire(blocking=False):
