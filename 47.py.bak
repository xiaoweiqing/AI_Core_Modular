...
def voice_to_text_workflow():
    """Handles toggling SHORT voice recording on and off (Alt+G)."""
    ...
    if IS_RECORDING:
        ...
        try:
            ...
            print(f"Error stopping short rec stream: {e}")
        IS_RECORDING = False
        print(f"\n{Colors.GREEN}>> Short recording stopped.{Colors.ENDC}")
        safe_notification("Recording Stopped", "Processing audio...")
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as fp:
            sf.write(fp.name, np.concatenate(short_rec_frames, axis=0), 16000)
            # We call the new helper with is_meeting=False
            threading.Thread(
                target=transcribe_audio_task, args=(fp.name, False)
            ).start()
        return
    if not app_controller_lock.acquire(blocking=False):
        safe_notification("System Busy", "Another task is in progress.")
        return
    try:
        IS_RECORDING = True
        short_rec_frames = []
        ...
        safe_notification("Recording Started", "Press Alt+G again to stop.")
        ...
        print(f"❌ {Colors.RED}[Voice Start Error] {e}{Colors.ENDC}")
        IS_RECORDING = False
...
def processor_task():
    """Background thread that transcribes audio files from the queue."""
    while True:
        try:
            path = transcription_queue.get()
            is_meeting_chunk = "rec_" in Path(path).name
            print(
...
def read_text_aloud(text):
    """
    Master function for gTTS (Online).
