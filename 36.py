...
# --- [NEW] C++ Bridge Integration ---
# This section allows the script to directly use the C++ llama.cpp bridge
# we built, bypassing the need for a separate API server.

# 默认设置为 False，我们稍后在主程序中决定是否启用
CPP_BRIDGE_ENABLED = False
try:
    # 动态地将 build-final 目录添加到路径中
    script_dir = Path(__file__).parent
    build_dir = Path("/home/weiyubin/projects/cpp_python_bridge/build-final")
    if build_dir.exists():
        ...
        print(
            f"✅ {Colors.GREEN}[C++ Bridge] Found local 'my_bridge' module.{Colors.ENDC}"
        )
    else:
...
class MyBridgeLLM:
    """A LangChain-compatible wrapper for our C++ LlamaEngine."""

    def __init__(self, model_path, n_gpu_layers, n_ctx):
        if not CPP_BRIDGE_AVAILABLE:
            raise RuntimeError("my_bridge C++ module is not available.")

        print(
            f"{Colors.BLUE}>> [C++ Bridge] Initializing LlamaEngine directly...{Colors.ENDC}"
        )
        self.engine = my_bridge.LlamaEngine(model_path, n_gpu_layers, n_ctx)
        print(f"✅ {Colors.GREEN}[C++ Bridge] LlamaEngine is ready.{Colors.ENDC}")

    # 【【【 核心修改 】】】
    # 让 invoke 方法能够“感知”到所有 C++ 层的参数
    # 这样既可以使用 C++ 的默认值，也可以在需要时从 Python 动态覆盖它们
    def invoke(self, prompt, **kwargs):
        """
        Emulates the 'invoke' method of LangChain's LLM classes,
        passing any additional keyword arguments to the C++ engine.
        """

        # 将 Python 传来的关键字参数，直接透传给 C++ 的 generate 函数
        response_text = self.engine.generate(prompt, **kwargs)
...
# --- [NEW] Local Model Configuration for C++ Bridge ---
# 当 CPP_BRIDGE_ENABLED = True 时，将使用这些设置
LOCAL_MODEL_PATH = "/mnt/data/model/Qwen3-30B-A3B-Instruct-2507-UD-TQ1_0.gguf"
...
# --- 为本地模型修改 ---
# (找到并替换您脚本中旧的 setup_api 函数)
def setup_api():
    """Initializes the AI model, preferring the C++ bridge if enabled and available."""
    global llm

    if CPP_BRIDGE_ENABLED and CPP_BRIDGE_AVAILABLE:
        try:
            # 使用我们新的 LangChain 风格包装器
            llm = MyBridgeLLM(
            ...
            print(
                f"❌ {Colors.RED}[C++ Bridge Error] Failed to initialize LlamaEngine: {e}{Colors.ENDC}"
            )
            print(
                f"   {Colors.YELLOW}Falling back to API server connection...{Colors.ENDC}"
            )

    # --- 如果 C++ 桥接未启用或失败，则执行原始的 API 连接逻辑 ---
    local_api_url = "http://127.0.0.1:8087/v1"
    try:
        print(
            f"{Colors.BLUE}>> [API Client] Trying to connect to local model server: {local_api_url}{Colors.ENDC}"
        )
        llm = ChatOpenAI(
        ...
        print(
            f"✅ {Colors.GREEN}[API Client] Connection successful! Ready to use model via API.{Colors.ENDC}"
        )
        return True
    except Exception as e:
        print(
            f"❌ {Colors.RED}[API Client Error] Failed to connect to local model server.{Colors.ENDC}"
        )
        print(
            f"   {Colors.YELLOW}Please check if your local AI service (like LM Studio or llama-server) is running. Error: {e}{Colors.ENDC}"
        )
        return False
...
def run_ai_task(prompt):
    try:
        # 我们现在只需要简单地调用全局 llm 对象的 invoke 方法
        response = llm.invoke(prompt)

        # C++ 内部已经处理了状态重置，但我们仍然保留对模型输出的清理
...
if __name__ == "__main__":
    # --- 【【【 在这里设置开关！ 】】】 ---
    # 将此设置为 True 可直接使用 C++ 模块，无需 API 服务器
    # 将此设置为 False 可使用旧的 API 连接方式
