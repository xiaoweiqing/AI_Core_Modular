...
def read_text_aloud(text, is_background_task=False):
    """
    The single, master function for reading text aloud offline using Piper.
    It handles its own lock unless told it's a background task.
    """
    # If this is a standalone task (like Alt+R), it needs to acquire the lock.
    # If it's called from another function that already has the lock, we skip this.
    ...
    try:
        ...
        # This is the most robust method: using a pipe (stdin)
        ...
        # Only release the lock if this was a standalone task.
        if not is_background_task:
            app_controller_lock.release()


def translate_and_read_aloud(text):
    """
    A dedicated workflow for Alt+Q. Translates, copies, and reads aloud
    ...
    try:
        ...
        print(
            "\n"
            + Colors.BLUE
            + "[Translate & Read] Translating to English..."
            + Colors.ENDC
        )
        ...
        print(
            Colors.YELLOW
            + "--- [ Translation Complete | Copied to Clipboard ] ---"
            + Colors.ENDC
        )
        print(Colors.CYAN + final_text + Colors.ENDC)

        # --- Part 2: Reading Aloud ---
        # Call the master read function, telling it that we are already inside a lock.
        read_text_aloud(final_text, is_background_task=True)

    finally:
        # The lock is released only after both translation and reading are done.
        app_controller_lock.release()
    ...
    def __init__(self):
        self.function_map = {
            ...
            str(TRIGGER_FILES["translate_to_en"]): create_hotkey_handler(
                translate_and_read_aloud  # Call our new dedicated function
            ),
            # ### --------------------------------- ###
