...
def setup_local_llm():
    ...
    try:
        ...
        llm = ChatOpenAI(
            ...
            max_tokens=4096,
            # --- 【【【 核心修改：大幅延长超时时间 】】】 ---
            # 从 120秒 延长到 600秒 (10分钟)，给予慢速模型充足的响应时间
            request_timeout=600,
            max_retries=1,
        )
        llm.invoke("Hi")  # 保持一个快速的连接测试
...
async def process_and_update_in_background(db_id, raw_text):
    ...
    try:
        # 注意：这里我们不再需要 asyncio.wait_for，因为超时已在 llm 初始化时设置
...
# ==============================================================================
# 4. 主程序、信号处理与启动流程 (v2.5 队列增强版)
# ==============================================================================
async def background_worker(queue: asyncio.Queue):
    """
    这是一个后台工作者，它会从队列中逐一取出任务并处理。
    """
    print("   -> [后台工作者] 已启动，等待分析任务...")
    while True:
        try:
            # 从队列中获取任务，如果队列为空，会在这里等待
            db_id, raw_text = await queue.get()
            print(f"   -> [后台工作者] 从队列中获取到新任务 ID:{db_id}，开始处理...")
            await process_and_update_in_background(db_id, raw_text)
            # 标记任务完成
            queue.task_done()
        except Exception as e:
            print(f"   !! [后台工作者] 处理任务时发生严重错误: {e}")


async def trigger_capture_task(queue: asyncio.Queue):  # <<< 新增 queue 参数
    """
    异步任务触发器，现在它只负责快速捕捉并把任务放入队列。
    """
    ...
    try:
        ...
        if clipboard_content and not clipboard_content.isspace():
            ...
            if db_id:
                # --- 【【【 核心修改：不再直接创建任务，而是放入队列 】】】 ---
                await queue.put((db_id, clipboard_content))
                print(f"   -> [秒存] 任务 ID:{db_id} 已成功放入后台处理队列。")
        else:
            print("\n>> [系统] 剪贴板内容为空，已忽略。")
...
async def watch_for_trigger(queue: asyncio.Queue):  # <<< 新增 queue 参数
    """
    异步文件监视器，现在会将队列传递给触发器。
    """
    ...
    while True:
        try:
            if TRIGGER_FILE.exists():
                # 注意这里不再创建新任务，而是直接调用
                await trigger_capture_task(queue)
...
async def main_async():
    ...
    # 启动一个后台工作者任务
    worker_task = asyncio.create_task(background_worker(task_queue))

    full_trigger_path = TRIGGER_FILE.resolve()
    print("\n" + "=" * 70)
    print("  ✅ [系统就绪] MuseBox 后台服务已启动 (队列增强版 v2.5)")
    ...
    try:
        await watch_for_trigger(task_queue)
    except (KeyboardInterrupt, asyncio.CancelledError):
        print("\n>> [系统] 收到退出指令，正在关闭...")
        worker_task.cancel()  # 优雅地关闭后台工作者
